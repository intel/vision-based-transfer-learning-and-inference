...
do
   case $1 in
    ...
    --PRECISION)
        ...
        if [ $PRECISION = "Mixed_Precision" ]
           ...
           echo "Mixed Precision is set by user" 
        fi
        ;;
    --inference )
        INFERENCE=1
    ...
    --NUM_EPOCHS )
        ...
        ;;
    -h | --help )
         echo "Usage: resisc.sh [OPTIONS]"
         echo "OPTION includes:"
         echo "   --PRECISION - whether to use FP32 or Mixed_precision Options : [FP32(default),Mixed_precision]"
    ...
    * )
        ...
        echo "Usage: resisc.sh [OPTIONS]"
        echo "OPTION includes:"
        echo "   --PRECISION - whether to use bf16 or FP32 precision Options : [FP32(default),bf16]"
        echo "   --platform - To optimize for SPR : [None(default),SPR]"
        echo "   --inference - whether to run only inference"
        echo "   --cp  - Specify checkpoint directory for inference"
        echo "   --OUTPUT_DIR  - Specify output Directory need to be saved"
...
if [ $PLATFORM = "SPR" ]
 ...
 numactl -C $range -m 0 python resisc_tflearn.py --precision=$PRECISION --inference "$INFERENCE" \
  ...
  python resisc_tflearn.py --precision=$PRECISION --inference "$INFERENCE" --cp "$CHECKPOINT_FILE" --platform="$PLATFORM" \
